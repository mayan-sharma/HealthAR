...
checkColinear = (x1, y1, x2, y2, x3, y3, x4, y4) => {
  let slope1 = findSlope(x1, y1, x2, y2);
  let slope2 = findSlope(x2, y2, x3, y3);
  return (
    Math.atan(slope1) - 5 * (Math.pi / 180) <= Math.atan(slope2) ||
    Math.atan(slope1) + 5 * (Math.pi / 180) >= Math.atan(slope2)
  );
};
...
checkRightAngle = (x1, y1, x2, y2, x3, y3) => {
  let slope1 = findSlope(x1, y1, x2, y2);
  let slope2 = findSlope(x2, y2, x3, y3);
  var angle1 = slope1 === Infinity ? Math.pi : Math.abs(Math.atan(slope1));
  var angle2 = slope2 === Infinity ? Math.pi : Math.abs(Math.atan(slope2));
  return (
    angle1 - angle2 >= (85 * Math.pi) / 180 ||
    angle1 - angle2 <= (95 * Math.pi) / 180
  );
};
...
checkPose = keypointsArr => {
  ...
  if (
    checkColinear(x[12], y[12], x[14], y[14], x[16], y[16]) &&
    checkRightAngle(x[6], y[6], x[8], y[8], x[10], y[10])
  ) {
